...
class ProxyMiddleware(object):
    ...
    def process_request(self, request, spider):
        ...
        # ip = ip_ports[2][0]
        # port = ip_ports[2][1]
        # ip_list = ['https://%s:%s' % (ip[0], ip[1]) for ip in ip_ports]
        self.logger.debug('using proxy')
        # proxies = {
        #     'http': 'http://%s:%s' % (ip, port),
        #     'https': 'https://%s:%s' % (ip, port)
        # }
        r = requests.get('http://192.168.44.83:8817/proxy/free_ip/1')
        request.meta['proxy'] = 'https://{}'.format(json.loads(r.text)['data']['ip'])
    ...
    #     r_d = requests.get('http://127.0.0.1:8000/delete?ip={}'.format(ip))
    #     print('yes')
    #     print(r_d)
    # def process_response(self, request, response, spider):
